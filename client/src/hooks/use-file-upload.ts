...
const generateIdForFile = (file: File | FileMetadata): string => {
...
const createPreviewForFile = (file: File | FileMetadata): string | undefined => {
  if (file instanceof File) {
...
const mapInitialFiles = (files: Array<File | FileMetadata>): FileWithPreview[] =>
  files.map((file) => ({
    file,
    id: generateIdForFile(file),
    preview: createPreviewForFile(file),
  }));
...
export type FileUploadOptions = {
  ...
  initialFiles?: Array<File | FileMetadata>;
...
export const useFileUpload = (options: FileUploadOptions = {}): [FileUploadState, FileUploadActions] => {
  ...
  const replaceFile = useCallback(
    (id: string, newFile: File) => {
      setState((prev) => {
        const fileToUpdate = prev.files.find((file) => file.id === id);
        if (!fileToUpdate) {
          return prev;
        }

        if (
          fileToUpdate.preview &&
          fileToUpdate.file instanceof File &&
          fileToUpdate.file.type.startsWith('image/')
        ) {
          URL.revokeObjectURL(fileToUpdate.preview);
        }

        const updatedPreview = createPreview(newFile);
