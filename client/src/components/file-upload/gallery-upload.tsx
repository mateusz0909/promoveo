...
export default function GalleryUpload({
  ...
  const fileActionsRef = useRef<{ removeFile: (id: string) => void; replaceFile: (id: string, file: File) => void } | null>(null);

  // Create default images using FileMetadata type
  const defaultImages = useMemo<FileMetadata[]>(
    () => [
      {
        id: 'default-1',
        name: 'avatar-1.png',
  ...
  const initialEntries = useMemo(() => {
    ...
    return showDefaultImages ? defaultImages : [];
  }, [defaultImages, initialFiles, showDefaultImages]);
  ...
  const handleFilesAdded = useCallback(
    async (addedFiles: FileWithPreview[]) => {
      ...
      for (const fileItem of addedFiles) {
        ...
        if (validateFile) {
          try {
            const validationError = await validateFile(fileItem.file);
            if (validationError) {
  ...
  useEffect(() => {
    fileActionsRef.current = { removeFile, replaceFile };
