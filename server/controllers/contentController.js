...
const contentController = {
  ...
  async generateContent(req, res) {
    ...
    try {
      ...
      if (files && files.length > 0) {
        await Promise.allSettled(files.map(file => tmpService.removeEntry(file.path)));
      }
    }
  },
  ...
  async generateAndSave(req, res) {
    try {
      ...
      // Validate the generated ASO text structure
      if (!generatedAsoText || typeof generatedAsoText !== 'object') {
        console.error('generate-and-save: Invalid ASO text structure received from Gemini');
        return res.status(500).json({ error: 'Invalid content generated by AI service' });
      }

      // Check if required fields exist
      const requiredFields = ['title', 'subtitle', 'promotionalText', 'description', 'keywords', 'headings'];
      for (const field of requiredFields) {
        if (!generatedAsoText[field]) {
          console.error(`generate-and-save: Missing required field: ${field}`);
          return res.status(500).json({ error: `AI service returned incomplete content - missing ${field}` });
        }
      }

      // Validate headings array structure
      if (!Array.isArray(generatedAsoText.headings)) {
        console.error('generate-and-save: headings is not an array');
        return res.status(500).json({ error: 'AI service returned invalid headings structure' });
      }

      if (generatedAsoText.headings.length < uploadedFiles.length) {
        console.error(`generate-and-save: Not enough headings generated. Expected ${uploadedFiles.length}, got ${generatedAsoText.headings.length}`);
        return res.status(500).json({ error: 'AI service did not generate enough headings for all screenshots' });
      }

      // Validate each heading object
      for (let i = 0; i < uploadedFiles.length; i++) {
        const heading = generatedAsoText.headings[i];
        if (!heading || typeof heading !== 'object' || !heading.heading || !heading.subheading) {
          console.error(`generate-and-save: Invalid heading structure at index ${i}:`, heading);
          return res.status(500).json({ error: `AI service returned invalid heading structure for screenshot ${i + 1}` });
        }
      }
  ...
  async regenerateContentPart(req, res) {
    try {
      ...
      console.log('regenerate-content-part: Full request body:', req.body);
      const { appName, appDescription, imageDescriptions, contentType, currentContent, language = 'English' } = req.body;
      console.log('regenerate-content-part: Extracted language:', language);
...
Requirements:
- IMPORTANT:Generate a maximum 170 characters.
- Short, engaging hook
